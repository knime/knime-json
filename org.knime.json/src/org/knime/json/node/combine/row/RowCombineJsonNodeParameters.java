...
class RowCombineJsonNodeParameters implements NodeParameters {
    ...
    enum CollectionType {
        ...
        Array,
        @Label(value = "Collect into object with key", description = "The rows become an object within another object "
    ...
    enum RootType {
        ...
        OMIT_ROOT,
            @Label(value = "Add root object with key", description = "When selected, the object or the array is added "
    ...
    @Widget(title = "Collection type",
           description = "Configure how to collect the JSON values.")
    @RadioButtonsWidget
    @ValueReference(CollectionTypeRef.class)
    CollectionType m_collectType = CollectionType.Array;

    @Layout(CollectionSection.class)
    @Persist(configKey = RowCombineSettings.OBJECT_KEY_COLUMN)
    ...
    @Effect(predicate = ObjectKeyIsRowIDEffect.class, type = Effect.EffectType.SHOW)
    String m_objectKeyColumn = null;

    @Layout(CollectionSection.class)
    @Persist(configKey = RowCombineSettings.OBJECT_KEY_ROW_ID)
    @Widget(title = "Use Row ID as key",
           description = "Use the Row ID as the key for the object instead of a column value.")
    @ValueReference(ObjectKeyIsRowIDRef.class)
    @Effect(predicate = ObjectCollectionTypeEffect.class, type = Effect.EffectType.SHOW)
    boolean m_objectKeyIsRowID = true;
    ...
    interface CollectionTypeRef extends ParameterReference<CollectionType> {
    }
    ...
    static class RootTypeEffect implements EffectPredicateProvider {
        @Override
    ...
    static class ObjectCollectionTypeEffect implements EffectPredicateProvider {
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getEnum(CollectionTypeRef.class).isOneOf(CollectionType.Object);
        }
    }

    static class ObjectKeyIsRowIDEffect implements EffectPredicateProvider {
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getEnum(CollectionTypeRef.class).isOneOf(CollectionType.Object)
                    .and(i.getBoolean(ObjectKeyIsRowIDRef.class).isFalse());
        }
    }
    ...
    public static class JSONValueProvider extends CompatibleColumnsProvider {
    ...
    static class KeyValuePairPersistor implements NodeParametersPersistor<KeyValuePairSettings[]> {
        ...
        public void save(final KeyValuePairSettings[] obj, final NodeSettingsWO settings) {
            if (obj != null) {
                String[] keys = Arrays.stream(obj)
                    .map(kv -> kv.m_key)
                    .toArray(String[]::new);
                String[] values = Arrays.stream(obj)
                    .map(kv -> kv.m_value)
                    .toArray(String[]::new);
                settings.addStringArray(RowCombineSettings.KEYS, keys);
                settings.addStringArray(RowCombineSettings.VALUES, values);
            } else {
                settings.addStringArray(RowCombineSettings.KEYS, new String[0]);
                settings.addStringArray(RowCombineSettings.VALUES, new String[0]);
            }
        }
        ...
        public String[][] getConfigPaths() {
            return new String[][]{
                {RowCombineSettings.KEYS},
                {RowCombineSettings.VALUES}
            };
        }
    }
}
